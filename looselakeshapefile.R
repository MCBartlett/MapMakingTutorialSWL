...
if(!exists("gilford_island") || nrow(gilford_island) == 0) {
  ...
  loose_lake_coords <- matrix(c(
    -126.46469376426575, 50.77757149044518,   #NW
    -126.32111971884207, 50.77757149044518    # NE 
    -126.32111971884207, 50.73765,  # SE
    -126.46469376426575, 50.73765  # SW
    
  ), ncol = 2, byrow = TRUE)
  
  Loose_lake <- st_polygon(list(loose_lake_coords)) %>%
    st_sfc(crs = 4326) %>%
    st_sf() %>%
    mutate(name = "loose lake")
  
  method_used <- "Manual coordinate outline"
  print("Created approximate loose lake outline")
}
