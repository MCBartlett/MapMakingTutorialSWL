...
libs = c("dplyr", "sf","terra", "tidyterra", "leaflet","bcmaps", "ggplot2", "fishualize","geodata", "patchwork")
...
ggplot() +
  ...
  geom_spatvector(data = cad , fill = "#b3b3b3") +
  geom_spatvector(data = great_lakes, fill = "lightskyblue3") +
...
canadamap = ggplot() +
              ...
              coord_sf(xlim = c(cad_ext$xmin, cad_ext$xmax),
                       ylim = c(cad_ext$ymin, cad_ext$ymax))+
              theme_bw() +
              theme(panel.background = element_rect(fill = "lightskyblue3"),
...
Next, because we got rid of the graticule and background using theme_void() we are goin to add a north arrow, scale bar and some final layout changes.
...
geom_spatvector(data = bc, fill = "#e5e5e5")+
  ...
  theme(
    legend.position = "inside",
    legend.position.inside = c(0.85, 0.85),panel.background = element_rect(fill = "lightskyblue3"),
